...
format_source_redcap_data_dictionary <-
        function(path_to_redcap_data_dictionary_csv,
                ...
                DATA_01 <- somersaulteR::standardize_column_names(DATA_00)
                ...
                TARGET_COLUMNS <- c("PROJECT_ALIAS", "IDENTITY_ID", "FORM_NAME", "VARIABLE_FIELD_NAME", "PERMISSIBLE_VALUE_LABEL", "FIELD_LABEL", "FIELD_TYPE", "CHOICES_CALCULATIONS_OR_SLIDER_LABELS", "FIELD_NOTE")
                DATA_07 <-
                        DATA_06 %>%
                        mirCat::select_target_cols_first(TARGET_COLUMNS)

                ##Adding timestamp for IDENTITY_ID creation
                DATA_08 <-
                        DATA_07 %>%
                        somersaulteR::add_timestamp_column(new_col_name = "IDENTITY_TIMESTAMP")
