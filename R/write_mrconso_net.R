...
write_mrconso_net <-
        function(dataframe, phrase_col, path_to_lookup, path_to_umls_net_dir, sub_forward_slash = TRUE, trim_inner_space = TRUE, sub_underscore = TRUE, push_to_github = TRUE) {

                ##Converting column of query phrases to vector
                phrase_col <- enquo(phrase_col)
                phrases <- dataframe %>%
                                        dplyr::select(!!phrase_col) %>%
                                        dplyr::distinct() %>%
                                        unlist() %>%
                                        unname()

                ##Reporting metrics
                typewriteR::tell_me(crayon::blue("\tThis dataframe has", nrow(dataframe), "row/s and there are", length(phrases), "unique phrases to query."))
