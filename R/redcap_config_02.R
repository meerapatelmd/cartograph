...
key_stripped <-
        key %>%
        select(KEY_CONCEPT_NAME, CUI, STR) %>%
        group_by(KEY_CONCEPT_NAME) %>%
        filter(row_number() == 1) %>%
        ungroup()
...
inventory <- dplyr::bind_rows(endocrine_parsed,
                ...
                dplyr::left_join(key_stripped, by = c("VARIABLE_FIELD_NAME" = "KEY_CONCEPT_NAME")) %>%
        dplyr::left_join(key_stripped, by = c("PERMISSIBLE_VALUE_LABEL" = "KEY_CONCEPT_NAME"), suffix = c(".variable", ".pv")) %>%
        dplyr::filter_at(vars(starts_with("STR")), all_vars(is.na(.)))
